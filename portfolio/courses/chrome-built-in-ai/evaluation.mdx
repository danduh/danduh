...
- Drive the suite against `window.ai` from CI with a Playwright bridge, split across every PR, nightly, and local so real Nano runs where it actually can.
...
## Run it against window.ai in CI

Here's the catch that surprises everyone the first time. `window.ai` — the whole built-in AI surface — only exists inside a browser tab. It's not in Node. So your test runner can't `import` it, stub a fetch, and call it a day. You drive a real browser instead, and you run the model inside the page.
...
DeepEval and Ragas are fine tools that live in Python, where `window.ai` doesn't exist and never will. Without a bridge of their own, they can't reach the model. Skip them for on-device browser work.
...
:::caution The eval imports window.ai in Node and everything is undefined
Symptom: your test runner sees `LanguageModel` and `window.ai` as `undefined`, and every case errors before the model runs.

Cause: the built-in AI globals only exist inside a browser tab. Node has no `window.ai`, so a plain `import` can't reach it.
...
- `window.ai` lives only in a tab, so evaluate through a Playwright bridge; split PR, nightly, and local by what real Nano can run, and re-run the suite on every Chrome release.
