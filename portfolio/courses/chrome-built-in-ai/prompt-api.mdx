...
A session has a budget — `inputQuota`, usually around 4096 input tokens — and
`inputUsage` climbs as the conversation grows. Watch the gap. When a new prompt
...
an older draft. `inputUsage` and `inputQuota` are the whole vocabulary now.
...
the oldest turns, the history plus the new prompt won't fit `inputQuota`. Fix:
