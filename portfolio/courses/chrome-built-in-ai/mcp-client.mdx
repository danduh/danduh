...
- The bridge to on-device Nano is the same intent loop: a `responseFormat` schema, the tools in the system prompt, and a capped prompt → parse → call → feed-back cycle.
