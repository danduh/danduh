...
a `responseFormat` schema, a system prompt that lists `renderForm`. Then loop:
prompt, parse, run the tool, feed the result back. When the tool returns UI, render
it and hand back `content`. When the user submits, you get a choice — round-trip the
values back through the model for a confirmation, or skip the model entirely.
...
const session = await LanguageModel.create({
  ...
  responseFormat: INTENT_SCHEMA,           // { toolName, args, reply }
...
Chrome. Or use the hosted demo: **[Generative UI](https://windowai.danduh.me/generative-ui)**
(with its [API walkthrough](https://windowai.danduh.me/generative-ui/docs)).
...
- [Structured output & tool calling](./structured-output-and-tools) — the intent loop and `responseFormat` schema this all rides on.
- [Generative UI API reference](https://windowai.danduh.me/generative-ui/docs) — the hosted MCP Apps (SEP-1865) walkthrough.
