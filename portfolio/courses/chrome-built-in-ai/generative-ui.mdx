...
- Return interactive UI from a tool with the MCP Apps shape — `content` plus a nested `_meta.ui.resourceUri`, not a string.
...
nested `_meta.ui.resourceUri` (an `McpUiToolMeta` object) the host peels off to
render. The tool stashes its markup in a registry keyed by a `ui://` token and
returns the token. (The old flat `_meta["ui/resourceUri"]` — with a slash — is
deprecated; use the nested form.)
...
const TOOLS = {
  ...
  // PLUS a nested `_meta.ui.resourceUri` the host resolves to sandboxed markup.
  renderForm(args) {
...
// The `_meta.ui` object is an McpUiToolMeta — resourceUri lives inside it.
...
Here's `renderFormHtml` itself — every interpolated field escaped, and a real
`Content-Security-Policy` baked into the document so even inside the sandbox it
can't reach the network:

<CodeTabs>
```js title="demo.js"
// A small self-contained document: escaped fields, a locked-down CSP,
// and one line that posts the submitted values back up to the host.
function renderFormHtml(spec) {
  const title  = escapeHtml(spec.title || 'Quick form');
  const submit = escapeHtml(spec.submitLabel || 'Submit');
  const fields = spec.fields || [{ name: 'name', label: 'Your name', type: 'text' }];
  const rows = fields.map((f) =>
    '<label>' + escapeHtml(f.label || f.name) +
    '<input name="' + escapeHtml(f.name) + '" type="' + escapeHtml(f.type || 'text') + '"></label>'
  ).join('');
  return [
    '<!doctype html><meta charset="utf-8">',
    // The CSP: no network, no external anything — inline style/script only.
    '<meta http-equiv="Content-Security-Policy" content="' +
      "default-src 'none'; style-src 'unsafe-inline'; script-src 'unsafe-inline'\">",
    '<h2>' + title + '</h2>',
    '<form id="f">' + rows + '<button type="submit">' + submit + '</button></form>',
    '<script>document.getElementById("f").addEventListener("submit",(e)=>{',
    '  e.preventDefault();',
    '  const values={}; new FormData(e.target).forEach((v,k)=>values[k]=v);',
    '  parent.postMessage({jsonrpc:"2.0",method:"ui/submit",params:{values}},"*");',
    '},false);<\/script>',
  ].join('');
}
```
...
One branch does it, and one guard keeps you honest — `console.assert` only logs,
it never stops execution, so if you actually want the leak *blocked* you throw:
...
// The invariant, actually enforced: ui:// must never reach the model.
if (back.includes('ui://')) throw new Error('ui:// leaked into the model prompt');
```
```ts title="demo.ts"
...
That `ui/submit` is this demo's own message name — the MCP Apps spec defines no such
method; there the view posts a `tools/call` back to the host. The envelope and the
source check below are identical either way, so the demo keeps the shorter name.
...
that answers in HTML. Create the session the way you always do — declare its languages
with `expectedInputs`/`expectedOutputs`, and a system prompt that lists `renderForm`. Then loop: prompt with
...
never wakes the model: it calls `commitRecipeToPlan`, a tool whose `McpUiToolMeta`
sets `_meta.ui.visibility: ['app']` — `visibility` lists who can reach a tool
(`"model"`, `"app"`, or both), so `['app']` alone drops it from the model's catalog.
...
— `_meta` included — back into `prompt()`. Fix: feed back only `content`, and throw
on `text.includes('ui://')` so a regression fails loudly instead of only logging
(`console.assert` never halts execution).
:::
...
- A UI-returning tool answers with `content` plus a nested `_meta.ui.resourceUri` — text for the model, a token the host resolves to markup.
...
- It all rides the `LanguageModel` intent loop, and app-only tools (`_meta.ui.visibility: ['app']`) let the iframe act without a model round-trip.
