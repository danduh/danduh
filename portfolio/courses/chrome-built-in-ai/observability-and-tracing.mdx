...
One footgun before you read any of it. The Prompt API renamed its accounting fields: `inputUsage` became `contextUsage`, `inputQuota` became `contextWindow`, and `measureInputUsage()` became `measureContextUsage()`. MDN and the Extensions docs still show the old names, and some Chrome builds still expose them. So read defensively — `session.contextUsage ?? session.inputUsage` — and stop guessing which browser you're on.
...
function traceStream(api, op, session, run) {
  ...
  const finalize = (finish, errorName) => {
    ...
    try { // Prompt API only — drift-safe; getters can throw on a destroyed session
...
function traceStream(
  ...
  const finalize = (finish: AiFinish, errorName?: string): void => {
    ...
    if (errorName) span.errorName = errorName;
    try {
      const s = session as { contextUsage?: number; contextWindow?: number; inputUsage?: number; inputQuota?: number };
      span.contextUsage = s?.contextUsage ?? s?.inputUsage;
      span.contextWindow = s?.contextWindow ?? s?.inputQuota;
    } catch { /* session gone — skip the numbers */ }
    emit(span);
  };
...
Cause: one of three. You're tracing the Summarizer or Translator, which don't expose context accounting — that's Prompt API only. Or you read it after `destroy()`, and the getter threw. Or the build still uses the old `inputUsage` names.

Fix: read drift-safe with `session.contextUsage ?? session.inputUsage`, read it before you destroy the session, and expect it to be absent on every API except the Prompt API.
:::
...
- [The Prompt API (LanguageModel)](./prompt-api) — where `contextUsage` and `contextWindow` come from, and the naming drift you read around.
