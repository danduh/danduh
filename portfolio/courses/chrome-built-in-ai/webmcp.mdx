...
- Resolve the moving entry point — `document.modelContext ?? navigator.modelContext` — and feature-detect it before you touch it.
...
- The entry point is `document.modelContext`, with `navigator.modelContext` as the deprecated fallback — resolve both, feature-detect, bail to a banner.
