...
// Read-only tool: readOnlyHint: true. Returns an object, serialized back.
const listItems: ModelContextTool = {
  name: 'listItems',
  description: 'List the current cart contents. Read-only.',
  inputSchema: { type: 'object', properties: {}, additionalProperties: false },
  annotations: { readOnlyHint: true },
...
async function registerTools(mc: ModelContext): Promise<AbortController> {
  const controller = new AbortController();
  for (const tool of CART_TOOLS) {
    await mc.registerTool(tool, { signal: controller.signal });
  }
...
**[WebMCP Recipe Workbench](https://windowai.danduh.me/webmcp/webmcp-demo)** (with the
[API walkthrough](https://windowai.danduh.me/webmcp/webmcp-api-documentation)).
...
:::caution InvalidStateError: Duplicate tool name
Symptom: a second `registerTool` for the same name rejects with
`InvalidStateError: Duplicate tool name`. Cause: you registered that name twice
without aborting the first — in React, StrictMode double-invokes your effect.
Fix: the rejection is async, so `await` the call (or `.catch()` it) — a
synchronous `try/catch` won't see it. Keep one `AbortController`, abort the prior
registration before re-registering, and treat the duplicate as already-registered.
:::
...
don't ship it as a hard dependency. Register an origin-trial token to light it up on
...
Canary — the model narrates instead of emitting a tool call. Fix: drive the tools
with the intent loop, and keep a fence-stripping parse for the occasional fenced reply.
:::
...
- [WebMCP API walkthrough](https://windowai.danduh.me/webmcp/webmcp-api-documentation) — the hosted reference for `registerTool` and the descriptor shape.
