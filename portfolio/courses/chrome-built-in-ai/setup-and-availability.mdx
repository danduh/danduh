...
16389.0.0+) on a Chromebook Plus — a machine that can run Gemini Nano.
...
all. Summarizer, Translator, and Language Detector went flag-free on the open web
at Chrome 138; the Prompt API followed at Chrome 148. All of them download the
model the first time you call `create()`. Flags
...
- `"available"` — on disk and ready. `create()` returns in a fraction of a second.
...
stateDiagram-v2
  ...
  available --> downloadable: auto-purge, under 10 GB free or 30 days idle
  available --> unavailable: on-device AI switched off in Settings
```

One edge surprises everyone: `available` is not forever. Let free disk fall under
10 GB, or leave the model untouched for 30 days, and Chrome quietly purges it —
...
Notice the `expectedInputs` / `expectedOutputs` in the calls below. You declare
the input and output modality plus language up front so `availability()` and
`create()` can answer for the exact combination you'll actually use — a model
that can do English text is not the guarantee it can do, say, audio in.
...
for a few hundred milliseconds — for however long it takes to pull a few gigabytes over the
...
a fraction from 0 to 1. Multiply by 100 for a percentage. `e.total` exists too,
but it is always 1, so `e.loaded` is already the fraction you want.
...
const session = await LanguageModel.create({
  ...
  monitor(m) {
    m.addEventListener('downloadprogress', (e) => {
      // highlight-next-line
      const pct = Math.round(e.loaded * 100); // e.loaded is 0..1 (e.total is always 1)
...
- Desktop only: Windows 10 or 11, macOS 13 or newer, Linux, or ChromeOS (Platform 16389.0.0+) on a Chromebook Plus. No Android, no iOS.
- Around 22 GB of free disk to *start* the download. Chrome then purges the roughly 4 GB model if free space later drops under 10 GB — the auto-purge edge from the diagram above.
- A GPU with over 4 GB of VRAM — or the CPU path, which needs 16 GB of RAM *and* 4-plus CPU cores. It can run CPU-only; it is not GPU-only.
...
async function setupLanguageModel() {
  ...
  // downloadable / downloading / available all mean create() can proceed.
  // But starting the *download* needs a user gesture — call this from a click
  // handler, not on load — and wire a monitor so the first run shows progress
  // instead of hanging. (Once the model is on disk, create() needs no gesture.)
  const session = await LanguageModel.create({
    expectedInputs:  [{ type: 'text', languages: ['en'] }],
    expectedOutputs: [{ type: 'text', languages: ['en'] }],
    monitor(m) {
...
is a fraction from 0 to 1 — and `e.total` is always 1, so it won't rescale it for
you.
...
fallback. And do not poll `availability()` in a loop hoping it flips. It is
read-only — it never starts a download — so a tight loop just burns cycles and
changes nothing; only `create()` moves the state forward.
:::
...
Cause: Chrome auto-purged Gemini Nano — free disk dropped under 10 GB, or no
